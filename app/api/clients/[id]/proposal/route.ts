...
export async function GET(request:Request,{params}:Context){
  ...
  try{const {id}=await params;const db=getRawDb();const account=await db.prepare('SELECT name,organization_kind,status FROM accounts WHERE id=?').bind(id).first<{name:string;organization_kind:string;status:string}>();if(!account)return json({error:'Company not found.'},404);
    ...
    return json({proposal,history:history.results,archived:account.status==='archived',closed:!!closed&&!['active','planned','blocked'].includes(closed.status),actor:{name:actor.displayName,email:actor.email}});
