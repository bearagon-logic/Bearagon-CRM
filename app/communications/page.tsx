    ...
    <WorkNavigation current="/communications"/><div className="ops-intake-body">
      ...
      <div className="intake-workspace">
        <section className="intake-list" aria-label="Inquiry list">
          ...
          {visible.map((item) => <button key={item.id} className={`intake-row ${selected?.id === item.id ? "selected" : ""}`} aria-pressed={selected?.id === item.id} disabled={saving} onClick={() => { if (selected && JSON.stringify(selected) !== JSON.stringify(items.find((i) => i.id === selected.id)) && !window.confirm("Discard unsaved follow-up changes?")) return; setSelected({ ...item }); setNotice(""); }}><span className="intake-row-heading"><b>{item.companyName}</b><em className={`intake-status ${item.status}`}>{item.status === "qualified" ? "Lead" : item.status}</em></span><span>{item.contactName} · {item.source}</span><p>{item.summary}</p><span className="intake-row-footer"><span>{item.owner || "Unassigned"}</span><span className={item.status !== "closed" && item.followUpDate && item.followUpDate < today() ? "intake-overdue" : ""}>{item.followUpDate || "No follow-up date"}</span></span></button>)}
        </section>
