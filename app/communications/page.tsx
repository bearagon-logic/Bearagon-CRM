...
type Inquiry = { id: string; accountId: string; companyName: string; contactName: string; email: string; phone: string; source: string; sourceRef?: string | null; summary: string; status: string; owner: string; nextAction: string; followUpDate: string; resolution: string; createdAt: string; recordedBy: string; updatedAt: string };
...
export default function Communications() {
  ...
  const hasUnsavedFollowUp=!!selected&&JSON.stringify(selected)!==JSON.stringify(items.find(i=>i.id===selected.id));
  const visible = useMemo(() => items.filter((i) => i.status!=="new"&&
    ...
    && [i.companyName, i.contactName, i.summary, i.owner, inquirySourceLabel(i.source, i.sourceRef), i.sourceRef || ""].some((v) => v.toLowerCase().includes(query.toLowerCase()))), [items, filter, query]);
  function selectInquiry(item:Inquiry) {
    if(saving||hasUnsavedFollowUp&&!window.confirm("Discard unsaved follow-up changes?"))return;
    setSelected({...item});setNotice("");
    requestAnimationFrame(()=>document.getElementById('inquiry-details')?.scrollIntoView({behavior:'smooth',block:'start'}));
  }
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      setNotice(selected.status === "new" ? "Follow-up saved. This inquiry remains in Needs review until you classify it or change its status." : "Follow-up saved."); await refresh(selected.id);
  ...
  return <AppShell><main className="ops-intake-page work-view">
    ...
    <WorkNavigation current="/communications"/><div className="ops-intake-body">
      <IntakeFeeds key={feedRevision} onImported={()=>{void refresh().catch((e)=>setError(e.message));}} onReview={(r)=>{setReviewId(r.id);setForm({...empty,...r.event,source:r.event.source==="retell"?"phone":"website"});setError("");setOpen(true);}}
        renderReviews={(receipts,dismiss)=><section className="intake-review-list inbox-identity-review" aria-labelledby="needs-review-heading">
          <h2 id="needs-review-heading">Needs review <span>{receipts.length+needsReview.length}</span></h2>
          <p className="inbox-review-intro">New website and call inquiries. Review the message, then mark it as a lead, start the client workflow, or clear test / spam.</p>
          {!receipts.length&&!needsReview.length&&<p>{loading?'Loading new inquiries…':'No new inquiries waiting for review.'}</p>}
          {receipts.map(r=><article key={r.id}><div><span className="inbox-review-label">Contact details need review</span><b>{r.event.companyName||r.event.contactName||'Unidentified caller'}</b><small>{inquirySourceLabel(r.event.source, r.event.sourceRef)} · {r.event.email||r.event.phone||'Contact details missing'}</small><p>{r.event.summary}</p><small>{r.reason}</small></div><div><Button disabled={saving||hasUnsavedFollowUp} onClick={()=>{setReviewId(r.id);setForm({...empty,...r.event,source:r.event.source==='retell'?'phone':'website'});setError('');setOpen(true);}}>Review & link</Button><Button variant="ghost" disabled={saving||hasUnsavedFollowUp} onClick={()=>void dismiss(r)}>Dismiss</Button></div></article>)}
          {needsReview.map(item=><article key={item.id} className={selected?.id===item.id?'is-selected':''}><div><span className="inbox-review-label">New inquiry</span><b>{item.companyName}</b><small>{item.contactName} · {inquirySourceLabel(item.source, item.sourceRef)}</small><p>{item.summary}</p><small>{item.owner||'Unassigned'} · {item.followUpDate||'No follow-up date'}</small></div><div><Button variant="outline" disabled={saving} onClick={()=>selectInquiry(item)}>Review inquiry</Button><InquiryActions inquiry={item} disabled={saving||hasUnsavedFollowUp} onDone={message=>{setNotice(message);void refresh(selected?.id).catch(e=>setError(e.message));}}/></div></article>)}
        </section>}
      />
      ...
      <h2 className="inbox-messages-heading">Follow-ups & history</h2>
      <div className="intake-toolbar"><label className="intake-search"><Search aria-hidden="true"/><Input aria-label="Search follow-ups" placeholder="Search follow-ups or history" value={query} onChange={(e) => setQuery(e.target.value)} /></label><label>Show<select value={filter} onChange={(e) => setFilter(e.target.value)}><option value="open">Open follow-ups</option><option value="working">Working</option><option value="qualified">Leads</option><option value="overdue">Overdue follow-up</option><option value="closed">Closed</option><option value="junk">Test / spam</option><option value="all">All follow-ups & history</option></select></label><span>{visible.length} shown</span></div>
      <div className="intake-workspace">
        <section className="intake-list" aria-label="Inquiry list">
          {loading && <p className="intake-empty">Loading inquiries…</p>}
          {!loading && !visible.length && <div className="intake-empty"><Inbox aria-hidden="true"/><h2>{items.length ? "No matching follow-ups" : "A real inbox starts here"}</h2><p>{items.length ? "New inquiries are in Needs review above. Change the filter to find saved follow-ups or history." : "Record the next call or website message. We’ll link it to a company and keep its follow-up here."}</p></div>}
          {visible.map((item) => <button key={item.id} className={`intake-row ${selected?.id === item.id ? "selected" : ""}`} aria-pressed={selected?.id === item.id} disabled={saving} onClick={() => { if (selected && JSON.stringify(selected) !== JSON.stringify(items.find((i) => i.id === selected.id)) && !window.confirm("Discard unsaved follow-up changes?")) return; setSelected({ ...item }); setNotice(""); }}><span className="intake-row-heading"><b>{item.companyName}</b><em className={`intake-status ${item.status}`}>{item.status === "qualified" ? "Lead" : item.status}</em></span><span>{item.contactName} · {inquirySourceLabel(item.source, item.sourceRef)}</span><p>{item.summary}</p><span className="intake-row-footer"><span>{item.owner || "Unassigned"}</span><span className={item.status !== "closed" && item.followUpDate && item.followUpDate < today() ? "intake-overdue" : ""}>{item.followUpDate || "No follow-up date"}</span></span></button>)}
        </section>
        <section id="inquiry-details" className="intake-detail" aria-label="Inquiry details">{selected ? <form onSubmit={save}>
          <div className="intake-detail-heading"><small>{inquirySourceLabel(selected.source, selected.sourceRef)} · {selected.createdAt.slice(0, 10)}</small><h2>{selected.companyName}</h2><Link href={`/clients/${selected.accountId}`}>Open account <ArrowUpRight aria-hidden="true"/></Link></div>
          {selected.source === "website" && selected.sourceRef && <p style={{overflowWrap:"anywhere"}}><b>Source page:</b> {selected.sourceRef}</p>}
          ...
          <div className="form-grid"><label>Status<select value={selected.status} onChange={(e) => setSelected({ ...selected, status: e.target.value })}>{states.map((s) => <option key={s} value={s}>{s === "qualified" ? "Lead" : s[0].toUpperCase() + s.slice(1)}</option>)}</select></label><label>Owner<OwnerSelect value={selected.owner} onChange={(value) => setSelected({ ...selected, owner: value })}/></label><label>Next action<Input maxLength={500} value={selected.nextAction} onChange={(e) => setSelected({ ...selected, nextAction: e.target.value })}/></label><label>Follow-up date<Input type="date" value={selected.followUpDate} onChange={(e) => setSelected({ ...selected, followUpDate: e.target.value })}/></label><label className="intake-wide">Resolution / handoff note<Textarea required={selected.status === "closed"} value={selected.resolution} maxLength={2000} placeholder="Record the outcome or handoff. Required to close." onChange={(e) => setSelected({ ...selected, resolution: e.target.value })}/></label></div>
          <div className="intake-save">
            <span>Recorded by {selected.recordedBy}</span>
            <div className="intake-action-buttons"><Button disabled={saving} type="submit">{saving ? "Saving…" : "Save follow-up"}</Button>{selected.status!=="closed"?<InquiryActions inquiry={selected} disabled={saving||hasUnsavedFollowUp} onDone={message=>{setNotice(message);void refresh(selected.id).catch(e=>setError(e.message));}}/>:<InquiryCleanup inquiry={selected} disabled={saving||hasUnsavedFollowUp} onDone={message=>{setNotice(message);setSelected(null);void refresh().catch(e=>setError(e.message));}}/>}</div>
            <p className="intake-action-help">Save any follow-up edits before using Actions. Closed inquiries can be reopened using Status.</p>
          </div>
