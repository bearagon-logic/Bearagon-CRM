      ...
      <div className="intake-workspace">
        ...
        <section className="intake-detail" aria-label="Inquiry details">{selected ? <form onSubmit={save}>
          ...
          <div className="intake-save">
            ...
            <div className="intake-action-buttons"><Button disabled={saving} type="submit">{saving ? "Saving…" : "Save follow-up"}</Button><InquiryCleanup inquiry={selected} disabled={saving || JSON.stringify(selected)!==JSON.stringify(items.find(i=>i.id===selected.id))} onDone={message=>{setNotice(message);setSelected(null);void refresh().catch(e=>setError(e.message));}}/></div>
            <p className="intake-action-help">Save any follow-up edits before cleanup. Closed inquiries can be reopened using Status.</p>
          </div>
