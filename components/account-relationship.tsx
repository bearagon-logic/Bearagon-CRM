...
export function AccountRelationship({ accountId, onRelationshipChange, onScope, onDirty }: { accountId: string; onRelationshipChange?: (type: string) => void; onScope?:()=>void; onDirty?:(dirty:boolean)=>void }) {
  ...
  return <section className="panel relationship-panel"><div className="panelhead"><div><small>{account?.organizationKind === "internal" ? "INTERNAL OPERATIONS" : "SALES & FOLLOW-UP"}</small><h2>{account?.organizationKind === "internal" ? "Bearagon’s own workspace" : "Review inquiry & follow-up"}</h2></div></div>
    ...
    </div><p className="relationship-help">Sales stage is separate from delivery. Marking won records the client relationship, not quote acceptance. Marketing preferences apply to this contact across linked companies; no campaigns are sent from here.</p><Button disabled={saving}>{saving ? "Saving…" : "Save relationship"}</Button>{onScope&&<Button name="qualify" type="submit" disabled={saving}>Qualify & scope services →</Button>}</form>}
