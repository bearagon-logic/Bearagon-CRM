...
import { Palette, Sun, Moon, Sparkles, Radio, Disc3 } from 'lucide-react';
...
export function PlaidScene(){
  const pathname=usePathname(),[line,setLine]=useState(0),[sideB,setSideB]=useState(false),[boost,setBoost]=useState(false);
  ...
  return <section className={`plaid-scene${boost?' plaid-boost':''}`} aria-label="Plaid lounge"><img className="plaid-banner-art" src="/plaid-cipher-banner.png" alt="" aria-hidden="true"/><div className="plaid-stars" aria-hidden="true"/><div className="plaid-scene-copy"><small>BEARAGON AFTER HOURS / {district}</small><strong>{boost?'THEY’VE GONE TO PLAID.':'THE FUTURE CALLED.'}</strong><span>{boost?'Secret unlocked. Ludicrous style enabled.':'It wants its neon back.'}</span></div><div className="plaid-toys"><button type="button" title="Flip the cassette" aria-label="Flip the cassette" onClick={()=>setSideB(v=>!v)}><Disc3 aria-hidden="true"/><span>{sideB?'SIDE B / MIDNIGHT RUN':'SIDE A / NEON DREAMS'}</span></button><button type="button" className="plaid-cipher" title="Check in with Cipher" onClick={()=>setLine(v=>(v+1)%cipherLines.length)}><img src="/cipher-bearagon.png" alt=""/><span>CIPHER // NIGHT SHIFT<small>{cipherLines[line]}</small></span></button></div><div className="plaid-secret" role="status">{boost&&'Secret unlocked: ludicrous style.'}</div><Radio className="plaid-signal" aria-hidden="true"/></section>;
}
