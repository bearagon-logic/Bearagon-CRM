...
export function InquiryCleanup({inquiry,onDone,disabled=false,open:controlledOpen,onOpenChange}:{inquiry:{id:string;companyName:string;updatedAt:string};onDone:(message:string)=>void;disabled?:boolean;open?:boolean;onOpenChange?:(open:boolean)=>void}) {
  const [localOpen,setLocalOpen]=useState(false),[kind,setKind]=useState("test"),[reason,setReason]=useState(""),[archive,setArchive]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState("");
  const open=controlledOpen??localOpen;
  const setOpen=(value:boolean)=>{setLocalOpen(value);onOpenChange?.(value);};
  return <>{controlledOpen===undefined&&<Button type="button" variant="outline" disabled={disabled} onClick={()=>{setKind("test");setReason("");setArchive(false);setError("");setOpen(true);}}>Mark as test / spam</Button>}
