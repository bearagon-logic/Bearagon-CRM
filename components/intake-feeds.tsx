...
type State={configured:boolean;lastSyncedAt:string;lastError:string;reviews:IntakeReview[];health:{retellConfigured?:boolean;sources?:{source:string;last_received_at:string;total:number;notification_pending:number}[]}};
export function IntakeFeeds({onReview,onImported,renderReviews}:{onReview:(review:IntakeReview)=>void;onImported:()=>void;renderReviews?:(reviews:IntakeReview[],dismiss:(review:IntakeReview)=>Promise<void>)=>ReactNode}) {
