...
import {useDirtyHistory,discardCurrentHistoryDrafts} from './use-dirty-history';
...
import {AutomationBrief} from './automation-brief';
...
type ProposalHistoryDraft={version:number;draft:ScopeDraft;answers:string[];order:WorkOrder|null;step:number;setupStep:number;editingSetup:boolean;reference:string;contact:string;date:string;confirmed:boolean};
...
export function ProposalWorkspace({accountId, mode, onSaved, onDirty, onBusy, onDelivery, onScope, children, setupOnly=false}:{accountId:string;mode?:'services'|'delivery';onSaved?:(p:ProposalState)=>void;onDirty?:(dirty:boolean)=>void;onBusy?:(busy:boolean)=>void;onDelivery?:()=>void;onScope?:()=>void;children?:React.ReactNode;setupOnly?:boolean}) {
  ...
  const historyKey=`proposal:${accountId}:${mode||'standalone'}:${setupOnly?'shared':'all'}`;
  const [recoveredVersion,setRecoveredVersion]=useState<number|null>(null),[recoveryBlocked,setRecoveryBlocked]=useState(false),[recoveredEvidence,setRecoveredEvidence]=useState<WorkOrder|null>(null);
  ...
  const heading=useRef<HTMLHeadingElement>(null),loadedHistoryKey=useRef(''),loadGeneration=useRef(0);
  ...
  const dirty=scopeDirty||setupDirty||orderDirty||!!recoveredEvidence||!!proposal&&!proposal.acceptance&&!!(reference||contact||date||confirmed);
  useDirtyHistory({dirty:()=>dirty,retain:()=>{if(proposal&&draft)retainHistoryDraft<ProposalHistoryDraft>(historyKey,{version:recoveredVersion??proposal.version,draft,answers,order:order||recoveredEvidence,step,setupStep,editingSetup,reference,contact,date,confirmed});},discard:()=>{clearHistoryDraft(historyKey);if(proposal){setDraft(withDefaultPrices(proposal.draft,proposal));setAnswers(proposal.setup.answers);}setOrder(null);setReference('');setContact('');setDate('');setConfirmed(false);setRecoveredVersion(null);setRecoveryBlocked(false);setRecoveredEvidence(null);}});
  ...
  const accepted=!!proposal?.acceptance,locked=accepted||busy||recoveryBlocked;
  function apply(payload:Payload){loadedHistoryKey.current=historyKey;setData(prev=>({...prev,...payload}));setDraft(withDefaultPrices(payload.proposal.draft,payload.proposal));setAnswers(payload.proposal.setup.answers);setConfirmed(false);onSaved?.(payload.proposal);}
  async function load(recover=false){const generation=++loadGeneration.current;setBusy(true);setError('');try{await waitForHistorySave(historyKey);const response=await fetch(`/api/clients/${accountId}/proposal`,{cache:'no-store'});const payload=await response.json() as Payload;if(generation!==loadGeneration.current)return;if(!response.ok)throw new Error(payload.error);apply(payload);const recovered=recover?takeHistoryDraft<ProposalHistoryDraft>(historyKey):undefined;if(recovered){setDraft(recovered.draft);setAnswers(recovered.answers);setOrder(recovered.order);setStep(recovered.step);setSetupStep(recovered.setupStep);setEditingSetup(recovered.editingSetup);setReference(recovered.reference);setContact(recovered.contact);setDate(recovered.date);setConfirmed(recovered.confirmed);setRecoveredVersion(recovered.version);const changed=payload.proposal.version!==recovered.version||!!payload.closed;setRecoveryBlocked(changed);setRecoveredEvidence(changed?recovered.order:null);if(changed)setOrder(null);setNotice(changed?'Recovered your unsaved entries. The saved record has changed or delivery is closed; these entries cannot be saved. Copy what you need before reloading the saved record.':'Recovered your unsaved entries after browser navigation. Review them before saving.');}else{clearHistoryDraft(historyKey);setRecoveredVersion(null);setRecoveryBlocked(false);setRecoveredEvidence(null);setOrder(null);setReference('');setContact('');setDate('');setConfirmed(false);setEditingSetup(false);}}catch(e){if(generation===loadGeneration.current)setError(e instanceof Error?e.message:'Unable to load proposal.');}finally{if(generation===loadGeneration.current)setBusy(false);}}
  useEffect(()=>{void load(true);return()=>{loadGeneration.current++;};},[accountId]);
  useEffect(()=>{if(mode==='services'&&query.get('step')==='review')setStep(3);},[query.get('step'),mode]);
  useEffect(()=>{const target=query.get('focus');if(mode!=='delivery'||!data||!target||!['delivery-setup','delivery-build','delivery-requirements'].includes(target)&&!target.startsWith('delivery-order-'))return;const frame=requestAnimationFrame(()=>{const element=document.getElementById(target);element?.focus({preventScroll:true});element?.scrollIntoView({behavior:'instant',block:'start'});});return()=>cancelAnimationFrame(frame);},[query.get('focus'),mode,!!data]);
  useEffect(()=>{onBusy?.(busy);return()=>onBusy?.(false);},[busy,onBusy]);
  useEffect(()=>{onDirty?.(dirty);if(loadedHistoryKey.current===historyKey&&!dirty)clearHistoryDraft(historyKey);},[dirty,onDirty,historyKey]);
  useEffect(()=>{if(recoveredVersion===null)setNotice('');setStepErrors([]);heading.current?.focus();window.scrollTo({top:0,behavior:'instant'});},[step]);
  useEffect(()=>{
    const unload=(event:BeforeUnloadEvent)=>{if(dirty){event.preventDefault();event.returnValue='';}};
    const leave=(event:MouseEvent)=>{const anchor=(event.target as Element)?.closest('a[href]');if(dirty&&anchor&&!anchor.getAttribute('href')?.startsWith('#')){if(!window.confirm('Leave this page and discard unsaved changes?')){event.preventDefault();event.stopPropagation();}else discardCurrentHistoryDrafts();}};
  ...
  async function act(command:Omit<ProposalCommand,'expectedVersion'>,nextStep?:number){
    if(!proposal||busy)return;if(recoveryBlocked){setError('The saved record changed while this draft was away. Copy the recovered entries, then reload the saved record before making changes.');return false;}const finishHistorySave=beginHistorySave(historyKey);setBusy(true);setError('');setNotice('');
    try{const response=await fetch(`/api/clients/${accountId}/proposal`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...command,expectedVersion:recoveredVersion??proposal.version})});const payload=await response.json() as Payload;if(!response.ok)throw new Error(payload.error||'Save failed.');apply(payload);finishHistorySave(true);setRecoveredVersion(null);setRecoveryBlocked(false);setRecoveredEvidence(null);setOrder(null);setNotice(command.action==='save'?'Draft saved.':command.action==='setup'?'Setup saved. Work orders reference this revision.':command.action==='approve'?'Saved revision approved for quoting. Nothing was sent.':command.action==='order'?'External evidence recorded. No automation was executed.':'Acceptance recorded. Services and delivery work are ready.');if(nextStep!==undefined){if(mode==='services'&&nextStep===4)onDelivery?.();else setStep(nextStep);}return true;}
    catch(e){setError(e instanceof Error?e.message:'Unable to save.');}finally{finishHistorySave(false);setBusy(false);}return false;
  }
  ...
  return <Wrapper><section className="lane-page proposal-page">
    ...
    <div className="lane-body">{stepErrors.length>0&&<div ref={validationRef} tabIndex={-1} className="lane-error" role="alert"><b>Complete this step before continuing</b><ul>{stepErrors.map(e=><li key={e}>{e}</li>)}</ul></div>}
      ...
      {!data||!draft?<p role="status">{error?'The record has not loaded.':'Loading saved proposal…'}</p>:<>
        ...
        {notice&&<p role="status" className="proposal-notice">{notice}</p>}{recoveryBlocked&&<div className="proposal-notice"><p>Current saved scope and delivery locks remain in effect.</p><RecoveredDraft text={recoveryText({scope:draft,setup:answers,evidence:order||recoveredEvidence,acceptance:{contact,date,reference,confirmed}})}/><Button variant="outline" onClick={()=>{if(window.confirm('Discard recovered entries and load the latest saved record?'))void load();}}>Reload saved record</Button></div>}
