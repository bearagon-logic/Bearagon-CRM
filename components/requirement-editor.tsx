...
import {requirementIssue,unmetRequirements,type Requirement} from '@/lib/requirement-readiness';
...
export function RequirementEditor({task,tasks,proposal,closed,busy,error,onChange,onSave,onClose,onOpen}:{task:Requirement;tasks:Requirement[];proposal:ProposalState|null;closed:boolean;busy:boolean;error:string;onChange:(t:Requirement)=>void;onSave:(t:Requirement)=>Promise<void>;onClose:()=>void;onOpen:(t:Requirement)=>void}) {
  ...
  const issue=requirementIssue(task,tasks),evidence=requirementEvidence(task.templateKey,proposal);
  return <form className="requirement-editor" onSubmit={e=>{e.preventDefault();if(!busy&&!closed&&!issue)void onSave(task);}}>
    ...
    {!editing&&!closed?<DialogFooter><Button type="button" variant="outline" onClick={onClose}>Close</Button><Button type="button" onClick={()=>{onChange({...task,status:'in_progress'});setEditing(true);}}>Record progress instead</Button></DialogFooter>:<>
      ...
      <fieldset disabled={busy||closed}>
        ...
        {task.status==='blocked'&&<label>Blocker reason (required)<Textarea maxLength={3000} rows={3} value={task.blockedReason||''} onChange={e=>onChange({...task,blockedReason:e.target.value})}/></label>}
      </fieldset>
