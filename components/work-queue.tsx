...
export function WorkQueue() {
  ...
  return <AppShell><main className="lane-page">
    ...
    <div className="lane-body">
      ...
      <section className="lane-panel" aria-labelledby="queue-title"><div className="lane-panel-heading"><h2 id="queue-title">Next actions</h2><label className="lane-filter">Show<select value={filter} onChange={e => setFilter(e.target.value)}>{["All work", "Inquiry", "Delivery", "Approval"].map(v => <option key={v}>{v}</option>)}</select></label><label className="lane-filter">Owner<select value={owner} onChange={e=>setOwner(e.target.value)}>{['All owners',...new Set(items.map(i=>i.owner))].map(v=><option key={v}>{v}</option>)}</select></label><label className="lane-filter">Due<select value={timing} onChange={e=>setTiming(e.target.value)}>{['All dates','Overdue','No date'].map(v=><option key={v}>{v}</option>)}</select></label></div>
        {loading && <p role="status" className="lane-empty">Loading saved work…</p>}
        {!loading && visible.map(item => {const inquiry=inquiries.records.find(i=>`inquiry:${i.id}`===item.id);return <div className="queue-item" key={item.id}><Link className="lane-work-row" href={item.href}><span className="lane-kind">{item.kind}</span><div><strong>{item.company}</strong><p>{item.title}</p></div><span className="lane-owner">{item.owner}<small>{item.due ? `Due ${item.due}` : "No date set"}</small></span><ArrowUpRight aria-hidden="true" /></Link>{inquiry&&<details className="queue-row-menu"><summary aria-label={`Actions for ${item.company}`}>Actions</summary><InquiryCleanup inquiry={inquiry} onDone={message=>{setNotice(message);inquiries.refresh();}}/></details>}</div>;})}
