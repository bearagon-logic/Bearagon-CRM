...
import { queueItems, type InquiryRecord, type DeliveryRecord, type DecisionRecord, type QueueItem } from "@/lib/workspace-model";
...
export function WorkQueue() {
  ...
  const sources = [{ label: "Inquiries", href: "/communications", ...inquiries }, { label: "Onboarding", href: "/onboarding", ...deliveries }, { label: "Approvals", href: "/approvals", ...decisions }, {label:"Internal automations",href:"/operations",...internal}];
  ...
  return <AppShell><main className="lane-page work-view">
    ...
    <WorkNavigation current="/"/><div className="lane-body">
      ...
      <section className="lane-panel" aria-labelledby="queue-title"><div className="lane-panel-heading"><h2 id="queue-title">Next actions</h2><label className="lane-filter">Show<select value={filter} onChange={e => setFilter(e.target.value)}>{["All work", "Inquiry", "Lead", "Delivery", "Internal automation", "Approval"].map(v => <option key={v}>{v}</option>)}</select></label><label className="lane-filter">Owner<select value={owner} onChange={e=>setOwner(e.target.value)}>{['All owners',...new Set(items.map(i=>i.owner))].map(v=><option key={v}>{v}</option>)}</select></label><label className="lane-filter">Due<select value={timing} onChange={e=>setTiming(e.target.value)}>{['All dates','Overdue','No date'].map(v=><option key={v}>{v}</option>)}</select></label></div>
