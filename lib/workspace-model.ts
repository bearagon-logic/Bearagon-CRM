...
export function queueItems(inquiries: InquiryRecord[], deliveries: DeliveryRecord[], decisions: DecisionRecord[], today: string): QueueItem[] {
  ...
  const rows: QueueItem[] = [
    ...inquiries.filter(i => i.status !== "closed").map(i => ({ id: `inquiry:${i.id}`, company: i.companyName, title: i.nextAction || "Review inquiry and assign the next action", owner: i.owner || "Unassigned", due: i.followUpDate, kind: i.status === "qualified" ? "Lead" : "Inquiry", href: `/communications?inquiry=${encodeURIComponent(i.id)}`, priority: overdue(i.followUpDate) ? 0 : i.status === "new" ? 1 : 3 })),
    ...deliveries.filter(d => d.organizationKind!=='internal').filter(d => ["planned", "active", "blocked"].includes(d.status)).map(d => ({ id: `delivery:${d.id}`, company: d.accountName, title: d.readiness?.label || "Review saved delivery plan", owner: d.owner || "Unassigned", due: d.targetDate, kind: d.blocked > 0 || d.status === "blocked" ? "Blocked delivery" : "Delivery", href: `/onboarding/${encodeURIComponent(d.accountId)}?tab=delivery`, priority: d.blocked > 0 || d.status === "blocked" || overdue(d.targetDate) ? 0 : 3 })),
